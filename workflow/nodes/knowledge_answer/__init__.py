...
def _infer_question_type(user_query: str) -> str:
    normalized = user_query.lower().strip()
    if any(token in normalized for token in ("公式", "怎么算", "如何计算", "计算方式", "target_cpa", "target_roas", "bid")):
        return "formula"
...
def _build_code_location_fallback(module_name: str, module_hint: str, code_hits: list[dict[str, Any]]) -> str:
    ...
    if not lines:
        lines = [
            ...
            "3. 目前没有命中可直接定位的代码证据，建议补充更具体的函数名、文件名或调用链关键字。",
        ]
    return "\n".join(lines)
    ...
    *,
    ...
    if not points:
        return (
            ...
            " 当前没有命中足够的证据，建议补充更具体的业务词、字段名或代码符号后重试。"
        )
...
def run(service: Any, state: dict[str, Any]) -> dict[str, Any]:
    ...
    question_type = _infer_question_type(user_query)
    ...
    if llm_answer_text:
        if _is_code_location_query(user_query) and code_hits and not _answer_mentions_code_anchor(llm_answer_text, code_hits):
        ...
        if _is_code_location_query(user_query) and code_hits:
            final_answer = _build_code_location_fallback(module_name, module_hint, code_hits)
        else:
