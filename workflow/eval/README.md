...
- 模板文件：`workflow/eval/wiki_hybrid_weights.template.json`
...
```powershell
.\workflow\eval\run_retrieval_eval.ps1
```
...
`workflow/eval/results/latest_eval_report.json`
...
```powershell
python workflow/eval/export_wiki_chunks.py --wiki-dir docs/wiki/ad_engine
```

### PowerShell 方式

```powershell
.\workflow\eval\export_wiki_chunks.ps1
```

默认输出：

- `workflow/eval/results/wiki_chunk_export.json`
- `workflow/eval/results/wiki_chunk_export.md`
...
```powershell
python workflow/eval/run_code_retrieval_eval.py --config workflow/eval/config.code.template.json
```

或

```powershell
.\workflow\eval\run_code_retrieval_eval.ps1
```

默认输出：

- `workflow/eval/results/latest_code_eval_report.json`
...
```powershell
python workflow/eval/run_answer_eval.py --config workflow/eval/config.answer.template.json
```

或

```powershell
.\workflow\eval\run_answer_eval.ps1
```

代码专项与融合专项可通过配置切换：

```powershell
.\workflow\eval\run_answer_eval.ps1 -ConfigPath workflow/eval/config.answer.code.template.json
.\workflow\eval\run_answer_eval.ps1 -ConfigPath workflow/eval/config.answer.hybrid.template.json
.\workflow\eval\run_answer_eval.ps1 -ConfigPath workflow/eval/config.answer.hybrid.smoke5.template.json
```

默认输出：

- `workflow/eval/results/latest_answer_eval_report.json`
