...
.DESCRIPTION
Wrapper script for `workflow/eval/run_answer_eval.py`.
You can pass a custom config path and Python command.
#>

param(
    [string]$ConfigPath = "workflow/eval/config.answer.template.json",
...
try {
    ...
    $env:WORKFLOW_CODE_RETRIEVER_DIRS = "codes"

    # Export env vars used by workflow/eval/run_answer_eval.py
    ...
    & $PythonCommand "workflow/eval/run_answer_eval.py" "--config" $ConfigPath
    exit $LASTEXITCODE
}
finally {
    Pop-Location
