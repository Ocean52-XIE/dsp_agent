...
- 节点入口：`src/workflow/nodes/retrieve_code/__init__.py`
- 检索器实现：`src/workflow/nodes/retrieve_code/code_retriever.py`
- 工作流接入：`src/workflow/engine.py`
...
- `WORKFLOW_CODE_RETRIEVER_DIRS`：代码索引目录（逗号/分号分隔）
  - 未配置时默认索引仓库根目录 `domain/ad_engine/codes/`，不会默认扫描整个仓库
...
- 配置：`src/workflow/eval/config.code.template.json`
- 数据集：`domain/ad_engine/eval/datasets/ad_engine_code_retrieval_eval.jsonl`
- 脚本：`src/workflow/eval/run_code_retrieval_eval.py`
- 启动：`src/workflow/eval/run_code_retrieval_eval.ps1`
